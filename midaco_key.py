...
def run( problem, option, key ):
 ...
 # kc 190305 keylib is MIDACO_ID, beginning of key name
 keylib =  key.decode("utf-8").partition("(")[0] 
 #print (keylib)

 if (os.name == "posix"):  lib_name = "midacopy-" + keylib + ".so"  # Linux//Mac/Cygwin
 else:                     lib_name = "midacopy-" + keylib + ".dll" # Windows
